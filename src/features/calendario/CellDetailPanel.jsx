...
export default function CellDetailPanel({ fechaDesde, fechaHasta, fecha, filaKey, filaLabel, jugadores, puedeGuardar = true, onClose, onGuardado }) {
      ...
      <div className="modal-card" onClick={(e) => e.stopPropagation()}>
        ...
        {filaKey === "fisico" && (
          <>
            ...
            <div style={{ borderTop: "1px dashed #eee", marginBottom: 14 }} />
          </>
        )}
        ...
        {!celda.dividido && (
          <>
            ...
            <textarea defaultValue={celda.detalle} disabled={!puedeGuardar} onBlur={(e) => guardar({ detalle: e.target.value })} />

            {filaKey === "tactica" && (
              <>
                <div className="field-label">Imágenes de los ejercicios</div>
                {puedeGuardar && <input type="file" accept="image/*" onChange={subirImagenTactica} />}
                ...
                <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(150px,1fr))", gap: 6 }}>
                  {jugadores.map((j) => {
                    ...
                    return (
                      ...
                      >
                        <input type="checkbox" checked={listaEfectiva.includes(j._id)} disabled={!puedeGuardar} onChange={() => toggleJugador(j._id)} />
                        <span style={{ flex: 1 }}>
                          {j.nombre}
          ...
          celda.grupos.map((g, idx) => {
            ...
            return (
              <div key={idx} style={{ border: "1px solid #eee", borderRadius: 10, padding: 12, marginBottom: 12 }}>
                ...
                <textarea defaultValue={g.detalle} disabled={!puedeGuardar} onBlur={(e) => editarGrupo(idx, "detalle", e.target.value)} />
                ...
                <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(150px,1fr))", gap: 6 }}>
                  {jugadores.map((j) => {
                    ...
                    return (
                      ...
                      >
                        <input type="checkbox" checked={efectivo.includes(j._id)} disabled={!puedeGuardar} onChange={() => toggleJugadorGrupo(idx, j._id)} />
                        <span>
                          {j.nombre}
                          {disp.motivo && <span style={{ display: "block", fontSize: 10, color: "#a51b1b" }}>{disp.motivo}</span>}
                        </span>
