...
import { puedeAdministrar } from "../../utils/permisos";
...
export default function CalendarioPage() {
  ...
  const handleDiaClick = async (fecha, yaEsPartido) => {
    if (yaEsPartido) {
      ...
      if (campeonatos.length > 0) {
        ...
        const eleccion = prompt(`¿A qué campeonato pertenece este partido?\n${lista}\n\nEscribí el número, o un nombre nuevo si no está en la lista:`);
        if (eleccion) {
  ...
  return (
    <div>
      ...
      <CampeonatosPanel campeonatos={campeonatos} onCambio={setCampeonatos} puedeEliminar={esAdmin} />
